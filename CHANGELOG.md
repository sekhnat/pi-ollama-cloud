...
- Update the dev tree to Pi 0.86.0 (`@earendil-works/pi-coding-agent`, `pi-ai`, `pi-tui` in `package-lock.json`; peer ranges stay `*`). No source changes were required: the 0.84→0.86 audit found the extension only uses the built-in `openai-completions` adapter (so the `Context` → `TranscriptContext` stream-input change does not apply), both web tools already ship the TypeBox parameter schemas 0.86 now enforces at registration, and their JSON `details` payloads already satisfy the tightened `AgentToolResult` typing. Lint, typecheck, and the full unit suite pass against the 0.86.0 type surface; the new optional `promptCache` model metadata is intentionally unset (no authoritative Ollama Cloud KV-cache TTL source).
